...
def test_create_album(api_client, user, token):
    api_client.credentials(HTTP_AUTHORIZATION="Token " + token.key)
    url = reverse("album-list")
    data = {"artist_name": "New Artist", "album_name": "New Album", "year_of_release": 2024, "ranking": 4}
...
def test_update_album(api_client, user, token):
    ...
    album = Album.objects.create(artist_name="Test Artist", album_name="Test Album", year_of_release=2023, ranking=5)
    url = reverse("album-detail", args=[album.id])
    data = {"artist_name": "Updated Artist", "album_name": "Updated Album", "year_of_release": 2024, "ranking": 4}
